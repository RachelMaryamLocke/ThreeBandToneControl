...
% dBScaleLowVal = toneLowValue * 24 - 12; 
% LsG = db2mag(dBScaleLowVal);

LsG = 10^((toneLowValue * 24 - 12)/20);

% dBScaleMidVal = toneMidValue * 24 - 12; 
% PnG = db2mag(dBScaleMidVal);

PnG = 10^((toneMidValue * 24 - 12)/20);

% dBScaleHighVal = toneHighValue * 24 - 12; 
% HsG = db2mag(dBScaleHighVal);
...
s1 = sqrt(LsG); %creates a variable for tan(wc/2)

zrsLs = [ (LsG * tLs + s1), (LsG * tLs - s1) ];

plsLs = [ (tLs + s1), (tLs - s1) ];

% plsLs = [ (s1 + tLs + s1), (tLs - s1) ]; %BEFORE
%%
...
% zrsPn = [ (sPn * tPn + 1), (-2 * sPn * cPn), (sPn - PnG * tPn) ]; %BEFORE
